...
### 우리 플랫폼에서 GraphRAG를 추가로 활용하는 방법
...
## 부록 A. 우리 플랫폼에서 이미 재사용 가능한 것들

RAG 포탈을 "0에서부터" 만들 필요는 없습니다. 아래는 이 플랫폼에 이미 있는 것 중 그대로/거의 그대로 재사용 가능한 부분입니다.

| 재사용 가능 자산 | 위치 | 재사용 방식 |
|---|---|---|
| **문서 업로드 + 프로파일링 파이프라인** | `apps/backend/app/modules/source/` (`SourceData`/`SourceProfile`/`SourceSegment`) | CSV/Excel 실파싱은 이미 구현됨. 청킹 결과가 이미 `SourceSegment(segment_type=CHUNK)`로 저장되고 있어, RAG 인덱싱 대상 텍스트를 그대로 여기서 가져올 수 있음 |
| **청크 뷰어 UI** | `DocumentChunkViewerPage.tsx` (`/sources/{id}/chunks`) | 리스트+상세 패턴이 이미 완성돼 있어 "문서별 상세 분석" 화면의 뼈대로 그대로 재사용 |
| **근거(Evidence) 오프셋 하이라이트 패턴** | `EvidenceViewerPage.tsx`의 `renderHighlightedText()` | 문자 오프셋 기반 `<mark>` 하이라이트 — 검색 결과의 근거 위치 표시에 그대로 이식 가능 |
| **게시 그래프 + 계보(lineage)** | `app/modules/publish/` (`PublishedEntity`/`PublishedRelation`) | 각 팩트가 원천 문서 offset/quote까지 추적됨 — GraphRAG의 지식그래프 소스이자 인용 근거로 즉시 활용 가능 |
| **RAG 질의응답 워크스페이스(틀)** | `RagAnswerWorkspacePage.tsx`, `apps/backend/app/modules/mvp4/` | 인용 표시, "근거 부족" 처리 UX가 이미 설계됨. **단, 현재 검색/임베딩 로직은 전부 결정론적 mock**(`VectorAdapterStatus.FALLBACK_KEYWORD` 고정 반환) — 실제 임베딩 모델 연동이 필요한 부분 |
| **골든셋 평가 + 파이프라인 단계별 스코어링** | `app/modules/evaluation/` (wave-072) | RAG 검색 품질(precision/recall) 평가 하네스로 그대로 전용 가능 — "추출→검증→검수→게시" 퍼널 개념을 "파싱→청킹→검색→생성" 퍼널로 그대로 대응 가능 |
| **온톨로지 실DB (클래스/속성/관계)** | `app/modules/ontology/` (wave-073에서 거버넌스 검증까지 실DB 연동) | GraphRAG의 스키마 그라운딩 소스로 바로 활용 가능 |

### 실제로 새로 만들어야 하는 것 (현재 갭)

- **PDF 실파싱**: 현재 PDF는 "바이트를 그대로 디코드"하는 자리표시자(placeholder) 구현입니다 (`source/router.py`의 "thin slice" 경고 주석). 실제 PDF 텍스트/표 추출이 필요합니다 (위 1절의 Docling/Unstructured 등).
- **의미 기반 청킹**: 현재 청킹은 고정 글자수(character count) 기반입니다. 토큰 기반 + 문단/구조 인식 청킹으로 교체가 필요합니다.
- **실제 임베딩 모델 연동**: 현재 mvp4 검색/RAG 모듈은 전부 하드코딩된 mock 응답입니다. 벡터 DB(예: Qdrant, pgvector)와 임베딩 모델(BGE-M3 등) 연동이 필요합니다.
- **실제 LLM 답변 생성**: 현재 RAG 답변도 결정론적 mock입니다.

## 부록 B. "문서별 상세 분석" 기능이 보여줄 내용 (설계 메모)

새 RAG 포탈 페이지에서 프로젝트의 특정 업로드 문서를 선택하면:

1. 파일 메타데이터(형식, 크기, 파싱 상태)
2. **현재 파싱 방식 진단**: 이 문서 형식에 이 플랫폼이 실제 구조 파싱을 지원하는지, 아니면 자리표시자인지 명시적으로 표시(PDF는 "⚠️ 자리표시자 파싱" 배지)
3. 청크 개수/타입 분포, 청크 크기 통계(평균/최소/최대 글자 수)
4. 샘플 청크 미리보기 (기존 `DocumentChunkViewerPage` 패턴 재사용)
5. 이 문서에 대한 권장사항 (예: "표가 많은 문서로 보입니다 — 레이아웃 인식 파서를 권장합니다" 같은 휴리스틱 기반 제안)
