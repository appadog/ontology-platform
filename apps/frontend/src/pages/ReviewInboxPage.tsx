    ...
    <PageContainer width="default">
      ...
      {tasks.length === 0 ? (
        ...
        <HanaCard title={`검수 대상 ${reviewTasksQuery.data.total_count}건`} description="워크벤치를 열기 전에 우선순위와 사유가 먼저 보입니다.">
          {/* Wave 64 (PM6-042 §2.6): mock's card-row hover treatment (rounded
              row + border-strong outline on hover), applied on top of the
              existing real table — all real columns are preserved. */}
